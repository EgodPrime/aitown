...
def test_effect_repo_crud_operations():
    conn = init_db(":memory:")
    repo = EffectRepository(conn)
...
def test_effect_repo_create_conflict():
    conn = init_db(":memory:")
    repo = EffectRepository(conn)
...
def test_effect_apply_to_npc_updates_attributes(monkeypatch):
    conn = init_db(":memory:")
...
def test_effect_apply_to_npc_updates_mood(monkeypatch):
    conn = init_db(":memory:")
    npc_repo = NpcRepository(conn)
    npc_repo.create(
        NPC(
...
def test_effect_apply_to_npc_ignores_unknown_attribute(monkeypatch):
    conn = init_db(":memory:")
    npc_repo = NpcRepository(conn)
    npc_repo.create(
        NPC(
