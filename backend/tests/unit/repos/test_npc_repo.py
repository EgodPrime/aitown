...
import aitown.repos.npc_repo as npc_mod


def test_npc_repo_crud_roundtrip(db_conn):
    conn = db_conn
    ...
    NpcRepository = npc_mod.NpcRepository
    NPC = npc_mod.NPC
    ...
    # db_conn fixture handles teardown
...
def test_remember_uses_module_memory_repo_and_summary_false(monkeypatch):
    ...
    npc = npc_mod.NPC(id="npc:fake", player_id=None, name="Fake")
...
def test_remember_with_none_and_explicit_repo(monkeypatch):
    ...
    npc = npc_mod.NPC(id="npc:both", name="B")
...
def test_create_npc_without_id_generates_uuid_and_inventory_none(db_conn):
    conn = db_conn
    ...
    conn.commit()
    repo = npc_mod.NpcRepository(conn)
    npc = npc_mod.NPC(
        id=None,
        player_id="p0",
        name="NGen",
...
def test_npc_notfound_delete_additional(db_conn):
    Repo = npc_mod.NpcRepository
    repo = Repo(db_conn)
...
def test_row_to_npc_inventory_none_and_json(db_conn):
    conn = db_conn
    # create player
    cur = conn.cursor()
    cur.execute(
...
def test_create_npc_with_zero_created_at_sets_time(db_conn):
    conn = db_conn
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO player (id, display_name, password_hash, created_at) VALUES (?,?,?,?)",
        ("player:1", "P1", None, time.time()),
    )
    conn.commit()

    repo = npc_mod.NpcRepository(conn)
    npc = npc_mod.NPC(id="npc:z", player_id="player:1", name="Z")
    created = repo.create(npc)
    assert created.created_at != 0


def test_npc_remember_and_summary_memory(monkeypatch, db_conn):
    ...
    repo = npc_mod.NpcRepository(conn)
    npc = npc_mod.NPC(id="npc:mem", player_id="player:1", name="Bob")
    repo.create(npc)
...
def test_remember_triggers_summary_when_long(monkeypatch):
    ...
    npc = npc_mod.NPC(id="npc:long", name="Long")
    npc.long_memory = "x" * 9001
