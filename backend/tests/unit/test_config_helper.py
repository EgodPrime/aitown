...
"""
    ...
    with patch.object(
        config_helper, "_load_toml", return_value={"section": {"key": "value"}}
    ):
        # First call
...
def test_get_config_full_config():
    ...
    with patch.object(
        config_helper,
        "_ensure_loaded",
        return_value={"section1": {"key1": "value1"}, "section2": {"key2": "value2"}},
    ):
...
def test_get_config_section_not_found():
    ...
    with patch.object(
        config_helper, "_ensure_loaded", return_value={"section1": {"key1": "value1"}}
    ):
        with pytest.raises(
            KeyError, match="Configuration section not found: nonexistent"
        ):
...
def test_get_config_section_not_dict():
    ...
    with patch.object(
        config_helper,
        "_ensure_loaded",
        return_value={"section1": {"key1": "value1"}, "section2": "scalar_value"},
    ):
